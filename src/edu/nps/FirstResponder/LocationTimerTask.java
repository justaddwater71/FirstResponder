...
public class LocationTimerTask extends TimerTask
{
	...
	public void run()
	{
		...
		if (curLoc != null)
		{
			...
			Log.i(FirstResponderParameters.DEB_TAG, "Sending location : " + lat
					+ ", " + lng);
			RestJsonClient.sendLocation(address, lat, lng);
			Log.i(FirstResponderParameters.DEB_TAG,
					"Location Timer Task Ended.");
		}
		...
		try
		{
			feedsJSON = RestJsonClient.getFeeds("http://"
					+ FirstResponderParameters.SERVER_HOST + "/feeds.json",
