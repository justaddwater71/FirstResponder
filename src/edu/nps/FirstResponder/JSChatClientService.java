	...
	public void connect(String urlString, int port, String username)
	{
		try
		{
			...
			} catch (JSONException e)
			{
				...
				//FIXME Put error INTENT here to let user know the server address is bad.....
			}
		...
		{
			connected=false;
			Log.e("IO", "Got IO Exception from Socket need to try to reconnect.", e);
			//FIXME Put intent here that let's user know we're broken for the moment.....
		}
	...
	while (true)
	{
		...
		try
		{
			if ((line = inConnection.readLine()) != null)
			{
				...
				intent = new Intent(FirstResponder.INTENT_ACTION_CHAT_MESSAGE);
				intent.putExtra(FirstResponder.INTENT_KEY_CHAT_MESSAGE, line);
